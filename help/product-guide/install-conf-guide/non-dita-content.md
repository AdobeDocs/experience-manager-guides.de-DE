...
### Konvertieren vorhandener Word-Dokumente in DITA-Themendokumente

Die folgenden Registerkarten enthalten Anweisungen zum Konvertieren vorhandener Word-Dokumente in DITA-Themendokumente basierend auf Ihrer Experience Manager Guides-Einrichtung: Cloud Service oder On-Premise.
...
1. Fügen Sie die Datei am folgenden Speicherort im Git-Repository Ihrer Cloud Manager hinzu:
...
Wenn Sie bereits AEM Guides as a Cloud Service verwenden und von der Version vom August 2021 auf die Version vom Januar 2022 oder höher aktualisieren, aktualisieren Sie die angegebenen Eigenschaften, da nur wenige Dateien verschoben wurden.

>[!NOTE]
>
> Dieses Update ist nur anwendbar, wenn Sie bereits den Konvertierungs-Workflow von Microsoft Word in DITA verwenden.
...
- Nehmen Sie die erforderlichen Änderungen in Ihrem Cloud Manager-Git-Repository vor, da für Cloud Service alle Dateien in /apps über Cloud Manager Git überlagert sind.
...
Im Großen und Ganzen erfordert der Konvertierungsprozess, dass Sie [InDesign-Dateien für die Konvertierung vorbereiten](aemg-appendix.md#id195DBF0045Z) und [die Zuordnungsdatei für die Migration von InDesign zu DITA vorbereiten](aemg-appendix.md#id194AF0003HT) anschließend müssen Sie das angegebene Verfahren zum Ausführen des Konvertierungsprozesses befolgen.
...
1. Erstellen Sie einen Überlagerungsknoten des `config` Ordners im `apps`.
   ...
   - Geben Sie im `mapStyle` den Speicherort der Zuordnungsdatei an, die Zuordnungen von InDesign-Dokumentstilen zu DITA-Elementen enthält. Die Standardzuordnung wird in der Datei gespeichert, die sich unter befindet:
     ...
     >[!NOTE]
     >
     > Weitere Informationen zur Struktur `stmap.adobeidml.xml` Datei und deren Anpassung finden Sie unter [Anhang](aemg-appendix.md#id194AF0003HT).
...
1. Laden Sie die InDesign-Quelldokumente in diesen Ordner hoch. Informationen zum Hochladen von Inhalten in DAM finden Sie unter [Vorhandenen DITA-Inhalt hochladen](upload-dita-content.md#).
...
### Konvertieren eines vorhandenen XHTML-Dokuments in ein DITA-Thema

Die folgenden Registerkarten enthalten Anweisungen zum Konvertieren Ihres vorhandenen XHTML-Dokuments in DITA-Themen basierend auf Ihrer Experience Manager Guides-Einrichtung: Cloud Service oder On-Premise.
...
1. *\(Optional\)* Sie können den Abschnitt „Verknüpfungen“ auch zu den konvertierten Dokumenten hinzufügen. Führen Sie die folgenden Schritte aus, um diese Funktion zu aktivieren:
   ...
   1. Verwenden Sie den Package Manager, um die Datei /libs/fmdita/html2dita/h2d.xsl herunterzuladen.
   ...
   4. Übertragen Sie die aktualisierte Datei in das Git-Repository Ihres Cloud Managers am folgenden Speicherort:
...
1. Laden Sie die Quell-XHTML-Dokumente in diesen Ordner hoch. Informationen zum Hochladen von Inhalten in DAM finden Sie unter [Vorhandenen DITA-Inhalt hochladen](upload-dita-content.md#).
...
Um ein vorhandenes XHTML-Dokument in DITA-Thema zu konvertieren, führen Sie die folgenden Schritte aus:
...
Um unstrukturierte Adobe FrameMaker-Inhalte (.fm und .book) in strukturierte DITA-Dateien zu konvertieren, können Sie den Konvertierungstabellenmechanismus von FrameMaker verwenden. Der Prozess konzentriert sich auf die Bewertung vorhandener Inhalte, die Verwendung eines vorlagenbasierten Ansatzes und die Zuordnung von FrameMaker-Stilen zu DITA über Konvertierungstabellen. Weitere Informationen finden Sie unter [Migration der technischen Dokumentation von unstrukturierten zu DITA in Adobe FrameMaker](https://migrate-from-unstructured-to-dita-step-by-step-guide.meetus.adobeevents.com/).

Nach der Konvertierung kann der strukturierte Inhalt nach AEM Guides migriert werden. Weitere Informationen finden Sie unter [Hochladen vorhandener DITA-Inhalte mit dem WebDAV-Tool und FrameMaker](./upload-dita-content.md).
...
Die folgenden Registerkarten enthalten Anweisungen zum Migrieren jedes anderen strukturierten Dokuments basierend auf Ihrer Experience Manager Guides-Einrichtung: Cloud Service oder On-Premise.
...
1. Verwenden Sie den Package Manager, um die Datei /libs/fmdita/config/XSLConfig.xml herunterzuladen.

1. Erstellen Sie eine Kopie der Datei „XSLConfig.xml“ am folgenden Speicherort im Git-Repository Ihres Cloud Managers:
...
1. Laden Sie die strukturierten Quelldokumente in diesen Ordner hoch. Informationen zum Hochladen von Inhalten in DAM finden Sie unter [Vorhandenen DITA-Inhalt hochladen](upload-dita-content.md#).
