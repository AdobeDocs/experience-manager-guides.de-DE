...
1. Erstellen Sie einen Überlagerungsknoten des `config` Ordners im `apps`.
   ...
   - Geben Sie im `mapStyle` den Speicherort der Zuordnungsdatei an, die Zuordnungen von InDesign-Dokumentstilen zu DITA-Elementen enthält. Die Standardzuordnung wird in der Datei gespeichert, die sich unter befindet:
     ...
     >[!NOTE]
     >
     > Weitere Informationen zur Struktur `stmap.adobeidml.xml` Datei und dazu, wie Sie sie anpassen können, finden Sie im Abschnitt [Appendix.md\#id194AF0003HT](appendix.md#id194AF0003HT) in Anhang.
...
Nach der Konvertierung kann der strukturierte Inhalt nach AEM Guides migriert werden.  Weitere Informationen finden Sie unter [Vorhandenen DITA-Inhalt hochladen](./migrate-content-upload-existing-dita-content.md).
