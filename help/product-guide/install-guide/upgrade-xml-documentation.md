...
Sie können Ihre aktuelle Version von Experience Manager Guides auf Version 5.0.0 aktualisieren:


- Wenn Sie Version 4.6.x, 4.6 oder 4.4 verwenden, können Sie direkt auf Version 5.0.0 aktualisieren.
- Wenn Sie Version 4.3.x, 4.2, 4.2.1 (Hotfix 4.2.1.3), 4.1 oder 4.1.x verwenden, müssen Sie ein Upgrade auf Version 4.4 durchführen, bevor Sie ein Upgrade auf Version 5.0.0 durchführen.
...
- [Upgrade auf Version 4.2](#upgrade-to-version-42)
- [Upgrade auf Version 4.2.1](#upgrade-to-version-421)
- [Upgrade auf Version 4.3.0](#upgrade-to-version-430)
- [Upgrade auf Version 4.3.1](#upgrade-to-version-431)
- [Upgrade auf Version 4.3.1.5](#upgrade-to-version-4315)
- [Upgrade auf Version 4.4.0](#upgrade-to-version-440)
- [Upgrade auf Version 4.6.0](#upgrade-to-version-460)
- [Upgrade auf Version 5.0.0](#upgrade-to-version-500)



>[!IMPORTANT]
>
> Bevor Sie mit dem Upgrade beginnen, sollten Sie eine vollständige Backup des Systems durchführen, um Datenverluste zu vermeiden.
...
1. Deinstallieren Sie alle Experience Manager Guides-Hotfixes, die auf der vorherigen Version (Haupt- oder Patch Version\) von Experience Manager Guides installiert wurden.
...
1. Laden Sie das Versionspaket 4.0 vom [Adobe Systems Software Distribution Portal](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html) herunter:

   - Wenn Sie eine UUID-Version der Software verwenden, suchen Sie &quot;4.0 UUID Release für XML Dokumentation Lösung für AEM 6.5&quot;.
   - Wenn Sie eine Nicht-UUID-Version der Software verwenden, suchen für &quot;4.0 Nicht-UUID-Release für XML Dokumentation Lösung für AEM 6.5&quot;.
Laden Sie das Paket mithilfe der CRX\-Package Manager auf den vorhandenen AEM Server Instanz\(s\) hoch und installieren Sie es.

   >[!NOTE]
   >
   > Warten Sie, bis alle Systemkomponenten Beginn.

1. Klar das Browsercache nach der Installation des Pakets.
1. Wenn ein Dispatcher auf AEM Autor Instanz konfiguriert ist, führen Sie die folgenden Schritte aus:
   - Stellen Sie sicher, dass Folgendes in den Dispatcher-Regeln behandelt wird:
...
1. Klar Dispatcher-Cache \(um alle `clientlibs` zwischengespeicherten zu löschen\).

## Aktualisierung auf Version 4.2

Ein Upgrade auf Version 4.2 hängt von der aktuellen Version von Experience Manager Guides ab.
...
1. Laden Sie das Paket der Version 4.2 vom [Adobe Systems Software Distribution Portal](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html) herunter.
...
1. Überprüfen **Sie DAM Update-Asset-arbeitsablauf** die entsprechenden Starter Konfigurationen. Gehen Sie dazu zur Workflow-Oberfläche AEM und öffnen Sie die Launcher.

   ```http
   http://localhost:4502/libs/cq/workflow/content/console.html
   ```

   Suchen Sie und nehmen Sie Änderungen \(falls erforderlich\) an den folgenden beiden Startern vor \(die aktiv sein sollten\), die DAM **Update-Asset-arbeitsablauf** entsprechen:
...
1. Starter für &quot;*Knoten erstellt*&quot; für **Workflow DAM-Update-Asset**- für `"jcr:content/jcr:mimeType!=video"` sollte der Wert „Globbing“ sein:
   ...
   - Der Wert für &quot;Globbing&quot; sollte wie folgt lauten:
...
1. Nachdem das Upgrade abgeschlossen ist, stellen Sie sicher, dass alle Anpassungen/Überlagerungen validiert und aktualisiert werden, damit sie dem neuen Anwendungs-Code entsprechen. Im Folgenden finden Sie einige Beispiele:
   ...
   - elementmapping.xml
   - ui\_config.json\(wurde möglicherweise im Ordner profiles festgelegt\)
   - Geändert `com.adobe.fmdita.config.ConfigManager`
   - Überprüfen Sie, ob der benutzerdefinierte Code alte Pfade verwendet hat \(wie im [Abschnitt Migrationszuordnung](#id2244LE040XA) erwähnt\) - sollte auf die neuen Pfade aktualisiert werden, damit die Anpassungen auch wie erwartet funktionieren.
1. Lesen Sie mehr über alle neuen Konfigurationen, die in der aktuellen Version eingeführt wurden \(überprüfen Sie [Versionshinweise](../release-info/release-notes-4-3.md)\) und prüfen Sie, ob ein Funktionen betroffen ist, und ergreifen Sie dann entsprechende Maßnahmen. Ein Beispiel könnte die Verwendung des in Version 4.0 eingeführten &quot;Verbesserten Umgang mit Dateien und Versionen&quot; sein, für den Sie eine Konfiguration aktivieren müssen.

## Schritte zum Indizieren der vorhandenen Inhalte für die Verwendung des neuen Suchens und Ersetzens:

Führen Sie die folgenden Schritte aus, um die vorhandene Inhalte zu indizieren, und verwenden Sie den neuen Text zum Suchen und Ersetzen auf Kartenebene:

- Führen Sie eine POST-Anfrage zum Server \(mit korrekter Authentifizierung\) - `http://<server:port\>/bin/guides/map-find/indexing`. \(Optional: Sie können bestimmte Pfade der Maps übergeben, um sie zu indizieren, standardmäßig werden alle Maps indiziert \|\| Beispiel : `https://<Server:port\>/bin/guides/map-find/indexing?paths=<map\_path\_in\_repository\>`\)
...
„Die *Abfrage* mehr als *100000 Knoten gelesen oder durchlaufen*. Um eine Beeinträchtigung anderer Aufgaben zu vermeiden, wurde die Verarbeitung gestoppt.&quot;

Dies kann passieren, weil der Index nicht ordnungsgemäß für die in der Aktualisierung verwendete Abfrage eingerichtet ist. Sie können die folgende Problemumgehung ausprobieren:
...
### Aktivieren Sie das Auslösen eines Skripts über ein Servlet (für 4.2.1)
...
In der obigen Antwort-JSON enthält der Schlüssel `lockNodePath` die Pfad zu den Knoten, die in der Repository erstellt wurden und auf den gesendeten Auftrag verweisen. Er wird automatisch gelöscht, sobald der Auftrag abgeschlossen ist, bis dahin können Sie sich auf dieser Knoten über den aktuellen Status des Auftrags informieren.

Beispielprotokoll:
Im Folgenden finden Sie ein Beispiel für Protokolle, die im Protokolldatei nach dem Auslösen des Skripts angezeigt werden.
...
>[!NOTE]
>
> Das dam-update-Medienelement-Modell kann angepasst werden. Wenn also Anpassungen vorgenommen wurden, müssen die Anpassungen und Experience Manager Guides in die Arbeitskopie des Modells Synchronisation.

1. **DAM Asset arbeitsablauf aktualisieren \(Post Änderungen werden verarbeitet\):**
...
1. Wählen Sie DAM Update-Asset-arbeitsablauf **aus**.
1. Klicken Sie auf **Bearbeiten**.
1. Wenn die **DXML Post Process Initiator**-Komponente vorhanden ist, stellen Sie sicher, dass die Anpassungen synchronisiert werden.
1. Wenn die **DXML- Post Prozessinitiatorkomponente** nicht vorhanden ist, führen Sie die folgenden Schritte durch, um sie einzufügen:

1. Klicken Sie auf **Einfügen Komponente** \(Verantwortlich für Experience Manager Guides Posting Verarbeitung als letzter Schritt im Prozess\).
1. Konfigurieren Sie den **Prozessschritt** mit den folgenden Details:
...
1. Launcher für &quot;*Node Erstellt*&quot; für **DAM Update-Asset arbeitsablauf** – für die Bedingung `"jcr:content/jcr:mimeType!=video"`sollte der Wert &quot;Globbing&quot; wie folgt lauten:
...
1. Auf Experience Manager Guides-Version 4.2 oder 4.2.x aktualisiert und den entsprechenden Installationsschritt abgeschlossen.
...
Nachdem Sie Experience Manager Guides installiert haben, können Sie die verschiedenen Konfigurationen, die von der neu installierten Version in Ihrem Setup anwendbar sind, zusammenführen.

## Schritte zum Posting Verarbeitung der vorhandenen Inhalte zur Verwendung des Berichts für fehlerhafte verknüpfen


Führen Sie die folgenden Schritte für die Posting Verarbeitung der vorhandenen Inhalte und Verwendung des neuen Berichts für defekte verknüpfen aus:

1. (Optional) Wenn mehr als 100.000 dita-Dateien im System vorhanden sind, aktualisieren Sie den `queryLimitReads` Wert under `org.apache.jackrabbit.oak.query.QueryEngineSettingsService` auf einen höheren Wert (einen Wert, der größer als die Anzahl der vorhandenen Assets ist, z. B. 200.000), und stellen Sie ihn dann erneut bereit.

   | PID | Eigenschaftsschlüssel | Eigenschaftswert |
   |---|---|---|
   | org.apache.jackrabbit.oak.query.QueryEngineSettingsService | queryLimitReads | Wert: 200000 <br> Standardmäßig Wert: 100000 |
...
### Aktivieren Sie das Auslösen eines Skripts über ein Servlet (für 4.3.1)
...
>[!NOTE]
>
> Das dam-update-Medienelement-Modell kann angepasst werden. Wenn also Anpassungen vorgenommen wurden, müssen wir die Anpassungen und Experience Manager Guides mit der Arbeitskopie des Modells synchronisieren.
...
1. Wählen Sie DAM Update-Asset-arbeitsablauf **aus**.
1. Klicken Sie auf **Bearbeiten**.
1. Wenn die **DXML- Post Prozessinitiator-Komponente** vorhanden ist, stellen Sie sicher, dass die Anpassungen synchronisiert werden.
1. Wenn die **DXML Post Process Initiator**-Komponente fehlt, führen Sie die folgenden Schritte aus, um sie einzufügen:
...
1. Konfigurieren Sie den **Prozessschritt** mit den folgenden Details:
   ...
   **Titel:** DXML Post Prozessinitiator
...
1. Führen Sie die folgenden APIs aus, um die Nachbearbeitung für alle Dateien auszuführen:
   ...
   |---|---|
   | Abfragetyp | **ERHALTEN** |
   | Parameter | jobId: Übergibt die jobId vom vorherigen Posting Anfrage. |
   | Erwartete Antwort | - Sobald der Auftrag abgeschlossen ist, reagiert das GET-Anfrage mit Erfolg. <br> - Falls Fehler auftreten, teilen Sie die Fehlerprotokolle zusammen mit der API-Ausgabe mit Ihrem Kundenerfolgs Team.  <br>Beispiel-URL: `http://<server:port>/bin/guides/reports/upgrade?jobId=2022/9/15/7/27/7dfa1271-981e-4617-b5a4-c18379f11c42_678` |
...
1. Konfigurieren Sie den **Prozessschritt** mit den folgenden Details:
   ...
   **Beschreibung**: DXML-Posting Initiator Schritt verarbeiten, der einen Sling-Auftrag für die DXML-Posting Verarbeitung der geänderten/erstellten Medienelement auslöst

   **Process Tab**

   - Wählen Sie aus der **Dropdown-Liste &quot;Process**&quot; den Eintrag **&quot;DXML Post Process Initiator**&quot; aus.
...
1. Zurück zur letzten Version zum standardmäßigen oder vorher vorhandenen Wert zurück, wenn `queryLimitReads` Sie ihn in Schritt 1 geändert haben.

### Aktivieren Sie die Auslösung eines Skripts über ein Servlet.

>[!NOTE]
>
> Wenn Sie ein Upgrade von 4.3.0 oder 4.3.1 durchführen, müssen Sie diese Schritte nicht ausführen.
...
In der obigen Antwort-JSON enthält der Schlüssel `lockNodePath` die Pfad zu den Knoten, die in der Repository erstellt wurden und auf den gesendeten Auftrag verweisen. Sie wird automatisch gelöscht, sobald der Auftrag abgeschlossen ist. Bis dahin können Sie in diesem Knoten den aktuellen Status des Auftrags abrufen.
...
Wenn Sie einen anderen benutzerdefinierten Sling Rewriter in Ihrer Codebasis haben, verwenden Sie einen `'order'` Wert größer als 50, da der Experience Manager Guides Sling Rewriter `'order'` 50 verwendet.  Um diesen Wert zu überschreiben, benötigen Sie einen Wert >50. Weitere Informationen finden Ansicht [Ausgabe Neuschreiben von Pipelines](https://sling.apache.org/documentation/bundles/output-rewriting-pipelines-org-apache-sling-rewriter.html).
...
Ein Upgrade auf Version 4.6.0 hängt von der aktuellen Version von Experience Manager Guides ab. Wenn Sie Version 4.4.0, 4.3.1, 4.3.0, 4.2 oder 4.2.1 (Hotfix 4.2.1.3) verwenden, können Sie direkt auf Version 4.6.0 aktualisieren.
...
1. Klicken Sie nach Abschluss der Änderungen oben rechts auf **Synchronisieren** . Sie erhalten eine Erfolgsmeldung Benachrichtigung.
   ...
   >[!NOTE]
   >
   > Aktualisieren und überprüfen Sie, ob benutzerdefinierte Änderungen und der Experience Manager Guides-Posting-Verarbeitungsschritt im endgültigen arbeitsablauf-Modell vorhanden sind.
...
1. Launcher für &quot;*Node Erstellt*&quot; für **DAM Update-Asset arbeitsablauf** – für die Bedingung `"jcr:content/jcr:mimeType!=video"`sollte der Wert &quot;Globbing&quot; wie folgt lauten:
   ...
   - &#39;excludeList&#39; sollte `"event-user-data:changedByWorkflowProcess"`haben.
   - Launcher für &quot;*Node Modified*&quot; für **DAM Update-Asset arbeitsablauf -** für die Bedingung &quot;`jcr:content/jcr:mimeType!=video`&quot; sollte der Wert &quot;Globbing&quot; lauten:

   ```json
   /content/dam(/((?!/subassets|/translation_output).)*/)renditions/original
   ```

   - `excludeList` sollte haben `"event-user-data:changedByWorkflowProcess"`.
...
1. Wenn Sie in damAssetLucene Anpassungen hinzugefügt haben, müssen Sie diese möglicherweise erneut anwenden. Nachdem Sie diese Änderungen vorgenommen haben, setzen Sie die Neuindizierung auf „true“. Dadurch werden alle vorhandenen Knoten mit den Anpassungen neu indiziert. Nach Abschluss wird das Neuindizierungs-Flag erneut auf „false“ gesetzt. Dies kann je nach Anzahl der Assets im System einige Stunden dauern.
...
2. Festlegen die Neuindizierung Eigenschaft als `true` (`false` standardmäßig) und klicken Sie auf Speichern **Alle**.
3. Nach Abschluss der Neuindizierung wird der Eigenschaft für Neuindizierungen auf `false` erneut festgelegt, und die Anzahl der Neuindizierungen wird um 1 erhöht.

   >[!NOTE]
   >
   > Dies kann je nach vorhandener Datenmenge einige Minuten dauern.
4. Führen Sie die gleichen Schritte für andere hinzugefügte oder geänderte Indizes aus: `guidesBulkActivation`, `guidesPeerLinkIndex`, und `guidesKonnectTemplateIndex`.

## Schritte zum Indizieren der vorhandenen Inhalte



Führen Sie die folgenden Schritte aus, um die vorhandene Inhalte zu indizieren:
...
>[!TIP]
>
> Die Aktualisierung auf Version 5.0.0 hängt von der aktuellen Version von Experience Manager Guides ab. Wenn Sie Version 4.6.3, 4.6.1, 4.6.0 oder 4.4 verwenden, können Sie direkt auf Version 5.0.0 aktualisieren.
...
1. Warten Sie nach Abschluss der Paketinstallation in den Protokollen auf die folgende(n) Meldung(en):
   ...
   - Fehler im Posting Implementierung Setup-Skripts
   - Ausnahme bei der Portierung des Übersetzungs-MAP
   - Die Übersetzungszuordnung kann für die Eigenschaft nicht von v1 auf v2 portiert werden
...
1. Überprüfen **Sie DAM Update-Asset-arbeitsablauf** die entsprechenden Starter Konfigurationen. Gehen Sie dazu zur Workflow-Benutzeroberfläche von AEM und öffnen Sie Starter.
...
## Schritte zum erneuten Indizieren der damAssetLucene-Datei

Indizieren Definition wird für damAssetLucene mit Guides aktualisiert. Informationen [ Neuindizierung von damAssetLucene nach ](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-16460) Aktualisierung auf Version 5.0.0 finden Sie in diesem Artikel .
