...
| Ausgabepfad/Zielpfad | Der Pfad innerhalb Ihres AEM-Repositorys, in dem die PDF gespeichert ist.<br>Der Ausgabepfad wird über die Variable `${base_output_path}` festgelegt, die vom Administrator konfiguriert wird. Um den Ausgabepfad zu konfigurieren, zeigen Sie [Basisausgabespeicherort für Cloud-Services konfigurieren](../native-pdf/configure-base-location-cs.md) oder [Basisausgabespeicherort für On-Premise-Services konfigurieren](../native-pdf/configure-base-output-location.md) basierend auf den Services an, die Sie verwenden.<br> Sie können auch einige vordefinierte Variablen verwenden, um den Ausgabepfad zu definieren, anzuzeigen [Variablen zum Festlegen der Optionen Zielpfad, Site-Name oder Dateiname verwenden](generate-output-use-variables.md#id18BUG70K05Z). |
